...
io.on('connection', function(socket){
  ...
  socket.on('register', (data) => {
    register({...data, socketId: socket.id}, socket);
  });
...
async function register(data: {username: string, password: string, socketId: string}, socket: socketIO.Socket) {
  console.log(data);
...
async function getUsers(socket: socketIO.Socket) {
  console.log('get users');
  const users = await UserModel.find();

  emit(socket, 'users', users);

}

function emit(socket: socketIO.Socket, event: string, data: any) {
